...
/**
 * FINAL CLIENT-SIDE CONFIG NORMALIZER
 * Fully robust, handles dirty configs, UI pages, type normalization
 */
...
// ─────────────────────────────────────────────
// TYPE NORMALIZATION (VERY IMPORTANT FIX)
// ─────────────────────────────────────────────
...
export function coerceBoolean(value, defaultValue = false) {
  ...
  if (typeof value === 'string') {
    ...
    if (['true', 'yes', '1', 'on', 'enabled'].includes(v)) return true;
    if (['false', 'no', '0', 'off', 'disabled'].includes(v)) return false;
  }
...
function coerceString(value, fallback = '') {
  if (typeof value === 'string' && value.trim()) return value.trim();
  if (value != null) return String(value).trim() || fallback;
  return fallback;
}
...
export function normalizeField(field) {
  ...
  return {
    ...
    required: coerceBoolean(field.required),
    label: coerceString(field.label, name)
  };
}
...
function normalizeEntity(raw) {
  ...
  const rawName = raw.name ?? raw.table ?? raw.model;
  const name = coerceString(rawName, '');

  if (!name) return null;

  const fields = coerceArray(raw.fields).map(normalizeField);
...
function resolveEntities(config) {
  ...
  for (const src of sources) {
    ...
    const entities = coerceArray(src)
      .map(normalizeEntity)
      .filter(Boolean);
...
export default function normalizeConfig(cfg) {
  ...
  // 🔥 FIX: PROPER UI PAGE NORMALIZATION
  let pages = [];

  const rawPages = cfg.ui?.pages || cfg.pages || [];

  if (Array.isArray(rawPages)) {
    pages = rawPages.map((p, index) => {
      if (!p || typeof p !== 'object') return null;

      const type = normalizeUIType(p.type);
      const table = normalizeName(p.table || p.entity);

      if (!type || !table) return null;

      const matchedEntity = entityMap[table];

      return {
        id: index,
        type, // 🔥 normalized (FORM → form)
        entity: matchedEntity?.name || table
      };
    }).filter(Boolean);
  }
  ...
  return {
    appName: coerceString(cfg.appName, 'Untitled App'),
    entities,
    pages, // 🔥 IMPORTANT
    auth: {
      enabled: coerceBoolean(cfg?.auth?.enabled, true)
    },
    warnings: []
  };
}