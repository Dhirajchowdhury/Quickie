...
import api from './api';
import normalizeConfig, { normalizeName } from './lib/normalizeConfig';
...
/**
 * Deduplicate + sanitize entities for sidebar rendering.
 * - Removes null / non-object entries
 * - Removes entries with blank names
 * - Removes case-insensitive duplicates (keeps first occurrence)
 */
function getSidebarEntities(entities) {
...
export default function App() {
    ...
    api.get('/config')
      .then((res) => {
        if (res.data.config) {
          ...
          // Auto-select first valid entity
          setActiveEntity(getSidebarEntities(normalized.entities)[0] ?? null);
        }
      })
      .catch(() => {});
  }, [token]);
  ...
  const sidebarEntities = getSidebarEntities(config?.entities);
    ...
    <div className="min-h-screen bg-[#f5f6fa] flex flex-col font-sans text-slate-900 antialiased">
      ...
      <header className="h-14 bg-white border-b border-slate-200/80 px-5 flex items-center justify-between flex-shrink-0 z-20 relative">
        ...
        <div className="flex items-center gap-2">
          {config && (
            <button
              onClick={() => { setConfig(null); setActiveEntity(null); }}
          ...
          <aside className="w-56 bg-white border-r border-slate-200/80 flex-shrink-0 flex flex-col">
            ...
            <nav className="flex-1 overflow-y-auto px-2.5 pb-4 space-y-0.5 scrollbar-thin">
              {sidebarEntities.length === 0 ? (
                ...
                sidebarEntities.map((entity, index) => {
                  const isActive =
                    normalizeName(entity.name) === normalizeName(activeEntity?.name);

                  return (
                    <button
                      key={`${normalizeName(entity.name)}_${index}`}
