...
// ── MainContent (ENTITY ONLY MODE) ───────────────────────
function MainContent({ activeEntity, toast }) {
  if (!activeEntity) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-8">
        <p className="text-sm text-slate-400">Select an entity</p>
      </div>
    );
  }
    ...
    api.get('/config')
      .then((res) => {
        if (res.data.config) {
          const normalized = normalizeConfig(res.data.config);
          setConfig(normalized);
          setActiveEntity(normalized.entities[0] || null);
        }
      })
      .catch(() => {});
  }, [token]);
    ...
    <div className="min-h-screen bg-[#f5f6fa] flex flex-col">
      ...
      <header className="h-14 bg-white border-b flex items-center justify-between px-5">
        ...
        <div className="flex gap-2">
          {config && (
            <button onClick={() => { setConfig(null); setActiveEntity(null); }}>
              Change config
            </button>
          )}
      ...
      {!config ? (
        ...
        <div className="flex flex-1">
          ...
          <main className="flex-1">
            <MainContent activeEntity={activeEntity} toast={toast} />
          </main>
        </div>
      )}
